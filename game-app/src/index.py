...
class MySokoban:
    def __init__(self):
        ...
        self.sreen = pygame.display.set_mode((gamemap_width, gamemap_height + self.part))
    ...
    def move(self, y, x):
        ...
        new_player_y = old_player_y + y
        new_player_x = old_player_x + x

        if self.map[new_player_y][new_player_x] == 1:
            return

        if self.map[new_player_y][new_player_x] in [3, 5]:
            new_ball_y = new_player_y + y
            new_ball_x = new_player_x + x
    ...
    def display_game(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ruutu = self.map[y][x]
                self.sreen.blit(self.images[ruutu], (x * self.part, y * self.part))
